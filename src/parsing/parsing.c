...
/*   Updated: 2023/07/13 22:49:27 by jlaurent         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_parameters(t_info *info, t_list *lst)
{
	...
	if (!info->pars.north_wall || !info->pars.south_wall || !info->pars.east_wall
		|| !info->pars.west_wall || info->pars.ceiling_color == (uint32_t)-1
		|| !info->pars.map || info->pars.floor_color == (uint32_t)-1)
		return (1);
	if ((int)info->pars.ceiling_color == -2 || (int)info->pars.floor_color == -2)
		return (2);
